...
void closestgroup (int nelems, float elems[][NFEAT], float cent[][NFEAT], int *grind)
{
   int g,j,i;
   double tdis, mind;
   #pragma omp parallel for private(j, i, tdis, mind, g) schedule(dynamic,2) num_threads(32)
   for(i = 0; i<nelems;i++){ 
      mind = DBL_MAX;
      for(j=0; j<NGROUPS;j++){
...
void groupcompactness (float elems[][NFEAT], struct ginfo *iingrs, float *compact)
{
   ...
   for(int i =0; i < NGROUPS; i++){
      ...
      if(telem < 2){
         ...
         cum = 0.0;
         cont = 0.0;
         #pragma omp parallel for private(j, k, a, o) reduction(+: cum, cont) schedule(dynamic,2) num_threads(32)
         for(j = 0; j < telem; j++){
...
void diseases (struct ginfo *iingrs, float dise[][TDISEASE], struct analysis *disepro)
{
   ...
   for(i = 0; i < TDISEASE; i++){
      ...
      for(j = 0; j < NGROUPS; j++){
         telem = iingrs[j].size;
         float pro[telem];
        #pragma omp parallel for private (a, k, tmedian) shared(pro, lmedian, bmedian, gmax, gmin) schedule(dynamic,2) num_threads(32)
            for( k = 0; k<telem; k++){ 
